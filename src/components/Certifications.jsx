function Certifications() {
  ...
  const textOnlyCerts = [
  'Cisco — CCNA 2: Switching, Routing, and Wireless Essentials (In Progress)',
  'Cisco — Cybersecurity Fundamentals (In Progress)',
  'Oracle — Java Fundamentals & Java Programming (Nov–Dec 2025)',
  'Google — Init.g: Software Engineering Foundations (Oct 2025)',
]
    ...
    <section className="px-6 md:px-20 py-24 bg-cream">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 mb-10">
            {certifications.map((cert) => (
                ...
                className="flex flex-col items-center gap-3 bg-surface/60 border border-charcoal/10 rounded-2xl p-5 hover:-translate-y-1 hover:shadow-md transition-all duration-300"
            >
                <img
                src={cert.badgeImage}
                alt={`${cert.name} badge`}
        ...
        <ul className="flex flex-col items-center gap-3 bg-surface/60 border border-charcoal/10 dark:border-charcoal/25 rounded-2xl p-5 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
            {textOnlyCerts.map((cert) => (
            <li
                key={cert}
